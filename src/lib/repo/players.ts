...
export async function getHornetsLast10Stats(season: number = DEFAULT_SEASON): Promise<DashboardData> {
  if (!supabaseAdmin) return { players: [], lastUpdated: new Date().toISOString() };
  ...
  if (error) {
    console.error('[supabase] last10 query error', error);
    return { players: [], lastUpdated: new Date().toISOString() };
  }
...
export async function getTeamRecentGames(season: number = DEFAULT_SEASON): Promise<{ games: RecentGame[]; lastUpdated: string }> {
  if (!supabaseAdmin) return { games: [], lastUpdated: new Date().toISOString() };
  ...
  if (error) {
    console.error('[supabase] recent-games query error', error);
